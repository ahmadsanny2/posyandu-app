<x-dynamic-component :component="auth()->user()->isParent() ? 'parent-layout' : 'app-layout'">
    ...
    <div class="max-w-2xl">
        ...
        <x-card title="Ubah Data Lansia" subtitle="Sesuaikan rincian informasi lansia: {{ $elderly->name }}">
            <form method="POST" action="{{ route('elderlies.update', $elderly->id) }}" class="space-y-4">
                @csrf
                @method('PATCH')
                ...
                <div>
                    ...
                    <x-text-input id="birth_date" name="birth_date" type="date" class="mt-1 block w-full" :value="old('birth_date', $elderly->birth_date)" required />
                ...
                <div>
                    ...
                    <textarea id="medical_history" name="medical_history" rows="3" class="mt-1 block w-full rounded-xl border-slate-200 text-sm focus:border-primary focus:ring focus:ring-blue-100 transition-shadow">{{ old('medical_history', $elderly->medical_history) }}</textarea>
