        ...
        <x-card title="Ubah Data Balita" subtitle="Sesuaikan rincian informasi balita: {{ $toddler->name }}">
            <form method="POST" action="{{ route('toddlers.update', $toddler->id) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="medical_history" name="medical_history" rows="3" class="mt-1 block w-full rounded-xl border-slate-200 text-sm focus:border-primary focus:ring focus:ring-blue-100 transition-shadow">{{ old('medical_history', $toddler->medical_history) }}</textarea>
