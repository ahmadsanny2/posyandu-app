<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <x-card title="Ubah Artikel Edukasi Kesehatan" subtitle="Sesuaikan materi atau gambar sampul artikel">
            <form method="POST" action="{{ route('articles.update', $article->id) }}" enctype="multipart/form-data" class="space-y-4">
                @csrf
                @method('PATCH')
                ...
                <!-- Cover Image -->
                <div>
                    <x-input-label for="image" value="Gambar Sampul Artikel (Pilih file jika ingin mengganti gambar)" />
                    @if($article->image_path)
                        <div class="my-2">
                            <p class="text-xs text-slate-400 mb-1">Gambar saat ini:</p>
                            <img src="{{ asset('storage/' . $article->image_path) }}" alt="{{ $article->title }}" class="h-24 w-36 object-cover rounded-xl border border-slate-200">
                        </div>
                    @endif
