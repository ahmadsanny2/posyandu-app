        ...
        <x-card title="Ubah Data Ibu Hamil" subtitle="Sesuaikan rincian informasi kehamilan: {{ $pregnantWoman->name }}">
            <form method="POST" action="{{ route('pregnant-women.update', $pregnantWoman->id) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="medical_history" name="medical_history" rows="3" class="mt-1 block w-full rounded-xl border-slate-200 text-sm focus:border-primary focus:ring focus:ring-blue-100 transition-shadow">{{ old('medical_history', $pregnantWoman->medical_history) }}</textarea>
