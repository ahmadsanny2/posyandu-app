<x-app-layout>
    ...
    <div class="max-w-2xl">
        ...
        <x-card title="Ubah Data Orang Tua" subtitle="Sesuaikan rincian akun: {{ $parent->name }}">
            <form method="POST" action="{{ route('parents.update', $parent->id) }}" class="space-y-4">
                @csrf
                @method('PATCH')
                ...
                <div>
                    ...
                    <x-text-input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email', $parent->email)" required />
