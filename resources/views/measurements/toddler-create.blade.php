    ...
    <div class="space-y-6">
        ...
        <x-card title="Input Pemeriksaan Balita" subtitle="Masukkan data KMS untuk: {{ $toddler->name }} pada kegiatan {{ $schedule->title }}">
            <form method="POST" action="{{ route('toddlers.measure.store') }}" class="space-y-6">
                ...
                <!-- Action Notes -->
                <div class="mt-4">
                    <x-input-label for="action_notes" value="Tindak Lanjut / Saran Gizi (Opsional)" />
                    <textarea id="action_notes" name="action_notes" rows="3" class="mt-1 block w-full rounded-xl border-slate-200 text-sm focus:border-primary focus:ring focus:ring-blue-100 transition-shadow" placeholder="Contoh: Berikan Makanan Tambahan (PMT) kaya protein, rujuk ke Puskesmas jika berat badan menurun bulan depan.">{{ old('action_notes') }}</textarea>
